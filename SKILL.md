...
- 用户给出论文**标题**、**arXiv/DOI/链接**、或直接 **PDF URL**（可多个）
...
**优先用 bundled 脚本**（解析 arXiv、DOI、直链 PDF、OpenAlex 标题检索）。
